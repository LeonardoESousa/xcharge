...
def arrhenius(Ea):
  ...
  return (v0)*np.exp(-Ea/(kB*T))
  
k_ann       = 1E7  #arrhenius(Ea_ann)
k_gen       = 1E7#arrhenius(Ea_gen)
...
k_formation = {(Li,Li):0, (Li,F):k_form, (F,Li):k_form, (F,F):0}
...
FP       = morphology.Create_ParticlesFP(num=10,pairs=generation_pairs)
#Vac       = morphology.Create_Particles('vacancy', 5, method, mat=[Li])
#Inter     = morphology.Create_Particles('interstitial', 5, method, mat=[F])

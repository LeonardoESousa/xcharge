...
h         = 4.135667e-15
kB        = 8.617333262e-5
Ea_ann_FP0 = 0.11 #0.11
Ea_ann_FP2 = 0.19 #0.11
Ea_gen    = 0.11
Ea_diss_V = 0.2
Ea_diss_I = 0.48
Ea_form_I = 0.25 #0.25 
...
def arrhenius(Ea):
  return np.exp(-Ea/(kB*T))#(kB*T/h)*np.exp(-Ea/(kB*T))
...
k_mig_V = 1/sum([1/arrhenius(E) for E in Ea_mig_V]) #effective rate for sequential event
k_form_I  = arrhenius(Ea_form_I)
k_form_V  = arrhenius(Ea_form_V)

print(k_mig_V,k_mig_I)
#print(k_ann,k_gen,k_dis,k_form,k_mig)
###DEFECTS PROCESSES######################################################################
...
k_diss = {Cs:k_dis, Cs_GB:k_dis, Br:k_dis, Br_GB:k_dis } # dissociation rates for each material combination
dissociation = DissociationFP(k=k_diss)

# FRENKEL PAIR ANNIHILATION
k_annihilation = {Cs:k_ann_FP0, Cs_GB:k_ann_FP0, Br:k_ann_FP0, Br_GB:k_ann_FP0} # annihilation rates for each material combination
annihilation_FP0 = Annihilation(k=k_annihilation)

k_annihilation = {Cs:k_ann_FP2, Cs_GB:k_ann_FP2, Br:k_ann_FP2, Br_GB:k_ann_FP2} # annihilation rates for each material combination
...
processes = {'vacancy':[migration_V, formation_V], 'interstitial':[migration_I], 'frenkelpair0':[], 'frenkelpair2':[]}
...
FP   = morphology.Create_ParticlesFP(num=40,pairs=generation_pairs) #ONLY CREATES FP2+
#########################################################################################

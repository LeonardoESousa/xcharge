...
class Forster_Annirad:
    ...
    def rate(self,**kwargs):
        ...
        ex_type = ex.species
        ss = [[p.position,mats[p.position],(ex_type,p.species)] for p in system.particles if ((ex_type,p.species) in list(system.annihi_radius.keys())) and (p.identity != ex.identity)]
        #print(ss)
        try:         
            for ele in ss:
                type1 = ele[2][0]

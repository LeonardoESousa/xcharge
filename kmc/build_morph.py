...
def lattice_saw(param):
    ...
    list_set = [[width_saw*x, width_saw*x+width_saw-1]
                for x in range(int(numy/width_saw)) if x % 2 == 0]  # left
    ...
    list_set = [[width_saw*x+width_saw, width_saw*x+width_saw-1+width_saw]
                for x in range(int(numy/width_saw)) if x % 2 == 0]  # right
    make_teeth(X_right, Y_right, Z_right, Mats_right,
               size_saw, width_saw, ps_right, list_set)
    ...
    X_right = -X_right + 2*X_max - \
        (X_max_teeth-X_max_noteeth) + r_min  # +2.2*X_max_noteeth

    cell_right = [[X_right[i], Y_right[i], Z_right[i], Mats_right[i]]
...
def load_cif(mol_file):
    ...
    with open(input_file) as f:
        ...
        for line in f:
            ...
            if(bond_part and (atom_keyword not in line) and (bond_keyword not in line)):
                ...
                # the atom indx that bounds with the origin_atom
                target_atom = int(line[2])
    ...
    if(symetric_case):
        ...
        neighbor_arr = []
        for i in range(N_at):
            neighbor_arr.append([atoms_array[i].index] +
                                atoms_array[i].neighbors)
    ...
    for mol_type in reduced_moltype_list:  # getting samples of each molecule type
        ...
        molecule_samples_indx = [i for i in range(
            len(molecular_types_int)) if molecular_types_int[i] == mol_type][0]
        ...
        with open(matfile, 'w') as file_mat:
            for moltype in bond_samples:
                ...
                for i in range(n_at):
                    ...
                    file_mat.write(format_origin_at % (
                        origin_atom_i) + '\t'.join([k for k in neigh_ele_arr_i]) + '\n')
                file_mat.write('\n')
    ...
    print("The unit cell with %i molecules has the dimensions of %f X %f X %f angstrons" % (
        len(center_masses_np), X_lenght, Y_lenght, Z_lenght))
    print()
    n_times = int(input(
        "Insert the number of times that you want to duplicate the unit cell (integer!):"))
    ...
    new_lattice = np.unique(new_lattice.round(
        decimals=5), axis=0)  # removing duplicates
    ...
    print("Now, the lattice, with %i molecules, has the dimensions of %f X %f X %f angstrons" % (
        len(new_lattice), X_lenght_mult, Y_lenght_mult, Z_lenght_mult))

    X_cm_mult = new_lattice[:, 1]
    Y_cm_mult = new_lattice[:, 2]
    Z_cm_mult = new_lattice[:, 3]
    # careful, others funcs have mats in the last position
    Mats = new_lattice[:, 0]
...
def parametrize_mat(par):
    ...
    unit_cell = [[X_unit[i], Y_unit[i], Z_unit[i], Mats_unit[i]]
...
def bilayer(par):
    ...
    shift_mat = input(
        "The cell on the right has %s types of molecules. Do you wish to relabel its indexes?(y/n)" % (len(non_duplicate_mat)))
    ...
    if(shift_mat == "y"):
        ...
        new_index = [str(int(item)) for item in input(
            "Enter the index (separated by space): ").split()]
        print(new_index)
    ...
    unit_cell_left = [[X_left[i], Y_left[i], Z_left[i], Mats_left[i]]
                      for i in range(len(X_left))]
    unit_cell_right = [[X_right[i], Y_right[i], Z_right[i],
                        Mats_right[i]] for i in range(len(X_right))]

    unit_cell_left = np.array(unit_cell_left)
    unit_cell_right = np.array(unit_cell_right)

    n_times_left = [n_times, n_times, n_times]
    multiplied_left = multiply_lattice(unit_cell_left, n_times_left, [
                                       dX_left, dY_left, dZ_left])
    ...
    multiplied_right = multiply_lattice(unit_cell_right, n_times_right, [
                                        dX_right, dY_right, dZ_right])
    ...
    # density of sites along an axis on the left side
    dens_left = dX_left/n_sites_axis_left
    # density of sites along an axis on the right side
    dens_right = dX_right/n_sites_axis_right
    ...
    def shift_hetero(latt, vec, indx_ar, mean, sigma, scale):
        ...
        for indx in indx_ar:
            shift = np.array(
                [scale*pos*np.random.normal(mean, sigma) for pos in vec])
            latt[indx][0:3] = latt_ref[indx][0:3] + shift

    disorder_on = input(
        'Do you wish to include disorder at the intersection?(y/n)')
    if disorder_on == 'y':
        disord_par = [str(int(item)) for item in input(
            "Enter the mean and the sigma of the noise (separated by space): ").split()]
        mean = disord_par[0]
        sigma = disord_par[1]
        scale = dens/1

        # getting the sites in the heterojunc
        left_het, right_het = is_in_the_heterojunc(
            multiplied_left, multiplied_right, dens)

        # shifting these sites
        shift_hetero(multiplied_left, vec_joint, left_het, mean, sigma, scale)
        shift_hetero(multiplied_right, vec_joint,
                     right_het, mean, sigma, scale)
...
# funcs option 2
saw_dir = ["Number of Sites", "displacement_vec", "distribuition_vec", "disorder",
           "saw_size", "width_size"]  # should have the same order of the function
# should have the same order of the function
lattice_dir = ["Number of Sites", "displacement_vec", "distribuition_vec"]
BHJ_dir = ["Number of Sites", "displacement_vec", "distribuition_vec", "Number of times to loop",
           "Cutoff distance (angstrom)"]  # should have the same order of the function
...
BHJ_func = morphology_function(
    "bulk heterojunction (BHJ)", lattice_BHJ, BHJ_dir)
loadcif_func = morphology_function("loadcif", load_cif, loadcif_dir)
# list of functions to be included in option 2
func_list = [latt_func, loadcif_func, BHJ_func, saw_func]

# funcs option 3
parametrize_dir = ["filename for the first unit cell"]
bilayer_dir = ["name_left_cell", "name_right_cell",
               "number_of_reps", "Joining axis (X,Y or Z)"]

paramet_func = morphology_function(
    "parametrized", parametrize_mat, parametrize_dir)
bilayer_func = morphology_function("bilayer", bilayer, bilayer_dir)

# list of functions to be included in option 3
func_opt3_list = [paramet_func, bilayer_func]
...
def main():
    ...
    if (choice1 == "2"):
        ...
        list_name_func = '\t'.join(
            [str(i)+"  "+str(list_name_func[i]) for i in range(n)])
        print("This version has %s pre-written morphology functions. Choose one: " %
              (len(func_list)))
        ...
        # list of parameters for a given func
        list_name_pars = '\t'.join([str(par) for par in par_list_name])
        print("This function has %s parameters. Namely," %
              (len(par_list_name)))
        ...
        for par_set in par_list_name:  # looping through each parameter
            print()
            print(par_set)
            # needed if a parameter is an array
            par_loc = [item for item in input(
                "Enter the parameter's entries (separated by space): ").split()]
            print(par_loc)
            par.append(par_loc)

        output_parameters = [(par_list_name[i], par[i])
                             for i in range(len(par_list_name))]
        ...
        write_lattice(lattice_output, func.name,
                      output_parameters, X, Y, Z, Mats, len(X))
